...
public class AuthController {
    ...
    public ApiResponseNormal<User> register(@RequestBody User user, @RequestParam String token) {
        System.out.println("register");
        if (authService.register(user, token)) {
            return new ApiResponseNormal<>(200, user, "注册成功");
        } else {
            return new ApiResponseNormal<>(400, null, "注册失败或无效通行口令");
        }
    }
